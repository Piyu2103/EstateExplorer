      ...
      <div className="m-4 flex flex-col md:flex-row max-w-6xl lg:mx-auto p-4 rounded-lg shadow-lg bg-white lg:space-x-5">
        <div className=" w-full h-[200px] lg:[400px]">
            <p className="text-2xl font-bold mb-3 text-blue-900 ">
                {listing?.name} - ${listing?.offer?parseInt(listing?.discountedPrice).toLocaleString():parseInt(listing?.regularPrice).toLocaleString()}
            ...
            <div className="flex justify-start items-center space-x-4 w-[75%]">
                ...
                {
                    listing?.offer && (
                        <p className="w-full max-w-[200px] bg-green-800 rounded-md p-1 text-white text-center font-semibold shadow-md">${(parseInt(listing?.regularPrice)-parseInt(listing?.discountedPrice)).toLocaleString()} discount</p>
                    )
                }
            </div>
            <p className="mt-3 mb-3">
               Description - <span className="font-semibold">{listing?.description}</span>
            </p>
